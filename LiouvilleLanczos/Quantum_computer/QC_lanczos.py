...
from qiskit.circuit.library import TwoLocal, RYGate, RZGate, CXGate, PauliEvolutionGate
...
from ..Lanczos_components import Inner_product as Base_inner_product,Summation as Base_summation, QKB_geometry
...
class QKB_2_loops(QKB_geometry):
    ...
    def make_circuit(self, i: int, j: int):
        ...
        qc.h(20)
        for target in [0, 2, 4, 6, 8, 11, 13, 15, 17, 19]:
            qc.cx(20, target)
        m = i
        n = j - i
        time_evol = PauliEvolutionGate(self.H, self.delta_t, synthesis = LieTrotter(reps = 1))
        for t in range(n):
...
class krylov_inner_product_spo(Base_inner_product):
    def __init__(self,
        ...
        self.estimator = estimator
        self.eps = epsilon
    def __call__(self, A: SparsePauliOp, B: SparsePauliOp, real_result: bool=False):
        """
        Computer the inner product between the A and B operator when the state is expressed in a D-dimensional Krylov basis $\\sum_j \\gamma_j \\ket{\\phi_j}$
        It is assumed that the $\\ket{\\phi_j}$ are generated by applying a trotterized time evolution operator $\\exp{-i H dt}$ j times
        Here, it is assumed that the control qubit for the Hadarmard test will be implemented as a control register of size n_checkpoints in a GHZ state. This is to make the
        second controlled preparation more hardware-efficient.
        The circuit prep_phi_0 should implement the beginning of the Hadamard test, aka it should prepare $\\ket{0}^n \\ket{0}^d + \\ket{1}^n\\ket{\\phi_0}$ where n is the size of the control register n_control
        The circuit prep_phi_0_on_0 should prepare $\\ket{\\phi_0}$ in the work register conditioned on the control register being in the all 0 state
        It is also assumed that the time evolution preserves excitation number and so acts trivially on the zero state
        The control register should be at the bottom of the circuit, i.e. the last n_checkpoints qubits
        """
        ...
        D = self.qkb.D
        #make observable
        n_qb = self.qkb.num_qubits
        ancillas_pos = self.qkb.ancillas_pos
        system_pos = self.qkb.system_pos
        #keep track of if any qubits are not ancillas or system (ex: used in preparing the state)
        extra_pos = []
        for i in range(n_qb):
            if i not in ancillas_pos and i not in system_pos:
                extra_pos.append(i)
        O = f
        #measure identity of superfluous qubits, XX...X and YX...X on ancillas, observable on system
        O_X = SparsePauliOp('I' * len(extra_pos)) ^ SparsePauliOp('X'*len(ancillas_pos), 1) ^ O
        O_Y = SparsePauliOp('I' * len(extra_pos)) ^ SparsePauliOp('Y' + 'X'* (len(ancillas_pos) - 1), 1) ^ O
        layout = system_pos + ancillas_pos + extra_pos
        O_X = O_X.apply_layout(layout)
        O_Y = O_Y.apply_layout(layout)

        O_X_real, O_X_img = separate_imag(O_X)
        O_Y_real, O_Y_img = separate_imag(O_Y)

        O_krylov = np.zeros([D,D], dtype = complex)
        for j in range(D):
            for i in range(j+1):
                #make circuit
                qc = self.qkb.make_circuit(i, j)
                res = self.estimator.run([(qc, [O_X_real, O_X_img, O_Y_real, O_Y_img])])
                res_ij = res.result()[0].data.evs
                O_krylov[i,j] = res_ij[0] + 1j * res_ij[1] + 1j * res_ij[2] - res_ij[3]
                O_krylov[j,i] = res_ij[0] + 1j * res_ij[1] - 1j * res_ij[2] + res_ij[3]

        gammas = self.qkb.gammas
        S = self.qkb.S
        ans = (gammas.conjugate().T @ O_krylov @ gammas) / (gammas.conjugate().T @ S @ gammas)
