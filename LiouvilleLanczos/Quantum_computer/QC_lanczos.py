...
def measure_obs_krylov_matrix_elements(obs: Operator, i: int, j: int, controlled_prep_circuit: QuantumCircuit, clifford_circuit: QuantumCircuit, estimator: BaseEstimatorV2, H: Operator, delta_t: float):
    #MEASURE <phi_i|obs|phi_j>
    circuits = []
    n_qubits = controlled_prep_circuit.num_qubits
    if(i - j >= 0):
        for i in range(delta_ij):
            qc = QuantumCircuit(n_qubits)
            qc.h(0)
            qc.append(controlled_prep_circuit, range(n_qubits))
...
class krylov_inner_product_spo(Base_inner_product):
    def __init__(self, prep_phi_0: QuantumCircuit, prep_phi_0_on_0: QuantumCircuit, estimator: BaseEstimatorV2, gammas: list[complex], overlap_matrix: np.ndarray[complex],  H: Operator, epsilon: int, dt: float, D: int, n_control: int):
        self.prep_phi_0 = prep_phi_0
        self.prep_phi_0_on_0 = prep_phi_0_on_0
        self.estimator = estimator
        self.eps = epsilon
        self.H = H
        self.D = D
    ...
    def __call__(self, A: SparsePauliOp, B: SparsePauliOp, real_result: bool=False, Name: Optional[str] = None):
        ...
        #make circuits
        n_qb = self.prep_phi_0.num_qubits
        time_evol = PauliEvolutionGate(self.H, self.dt, synthesis = LieTrotter(reps = 1))
        #I save the observable values to memory for debugging purposes
        O_ij = np.ndarray([10,10], dtype=complex)
        O = f
        O_X = SparsePauliOp('X'*self.n_control, 1) ^ O
        O_Y = SparsePauliOp('Y' + 'X'* (self.n_control - 1), 1) ^ O
        ...
        for j in range(self.D):
            for i in range(j+1):
                ...
                qc = self.prep_phi_0.copy()
                for t in range(n):
                    qc.append(time_evol, range(12))
                qc.append(self.prep_phi_0_on_0, range(n_qb))
                for t in range(m):
                    qc.append(time_evol, range(12))
                res = self.estimator.run([(qc, [O_X_real, O_X_img, O_Y_real, O_Y_img])])
                res_ij = res.result()[0].data.evs
                O_ij[i,j] = res_ij[0] + 1j * res_ij[1] + 1j * res_ij[2] - res_ij[3]
                O_ij[j,i] = res_ij[0] + 1j * res_ij[1] - 1j * res_ij[2] + res_ij[3]
        ans = (self.gammas.conjugate().T @ O_ij @ self.gammas) / (self.gammas.conjugate().T @ self.S @ self.gammas)
